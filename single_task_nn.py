...
from tensorflow.keras.layers import Dense, Dropout, Embedding, Bidirectional, LSTM, GRU, Conv1D, MaxPooling1D, GlobalAveragePooling1D, GlobalMaxPooling1D
...
"""
Parse arguments sent through the command line
"""
...
# 100, 500, 1000
parser.add_argument("--max_sent_len", type=int, default='1000', help="Max sentence length")
...
parser.add_argument("--pooling_layer", type=str, default='avg', help="Type of pooling layer")
...
# 0.01, 0.001, 0.0001
parser.add_argument("--regularizer", type=float, default='0.001', help="Lambda of L2 regularizer")
# Dense arguments
# 5, 50, 100
...
for i in range(k):
    ...
    if params.vocab_percent > 1 or params.vocab_percent < 0:
        sys.exit(0)
    ...
    if nn_type == 'MLP':
        if params.pooling_layer == 'avg':
            model.add(GlobalAveragePooling1D())
        elif params.pooling_layer == 'max':
            model.add(GlobalMaxPooling1D())
        model.add(Dense(units=params.dense_units, activation='relu', kernel_regularizer=l2(params.regularizer)))
        model.add(Dropout(params.dropout))
        for l in range(1, params.num_dense_layers):
            model.add(Dense(units=params.dense_units, activation='relu', kernel_regularizer=l2(params.regularizer)))
            model.add(Dropout(params.dropout))
    elif nn_type == 'LSTM':
